...
function navigateTo(screenName) {
  ...
  /*Posicionar FAB dentro del contenedor + live updates*/
...
window._updateGlobalFabPos = function() {
  ...
  const right  = window.innerWidth - rect.right + 20;  /* 20px del borde del app */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* 4. FAB global → abre chat (único listener, sin bugs) */
  ...
  if (savedSession && savedSession.emailVerified && savedSession.name) {
    ...
    if (activeToken && myTabToken !== activeToken) {
      navigateTo('welcome');
    } else {
...
window.OlympusXP = {
  ...
  /* Sincroniza AppState con los datos reales de localStorage */
  sync() {
    ...
    /* También sincronizar racha desde OlympusStats */
  ...
  add(amount, motivo = '') {
    ...
    /* Actualizar UI en tiempo real sin recargar pantalla */
    _liveUpdateXPBar();

    return { ...calc, gained: amount, leveledUp: d.level > oldLevel };
  },

  /* Agregar XP por racha si corresponde */
...
window._startLiveUpdates = function() {
  ...
  window._liveInterval = setInterval(() => {
    ...
    try {
      ...
      /* Stats del admin en tiempo real */
