...
/*init común: logout + nav*/
function _adminCommonInit() {
  /* Fullscreen */
  const app = document.getElementById('app');
  if (app) { app.style.maxWidth='100%'; app.style.height='auto'; app.style.minHeight='100vh'; app.style.overflow='auto'; }
  ...
  document.getElementById('adm-logout')?.addEventListener('click', () => {
    if (confirm('¿Cerrar sesión del panel de administrador?')) {
      const app=document.getElementById('app');
      if(app){app.style.maxWidth='430px';app.style.height='100vh';app.style.minHeight='';app.style.overflow='hidden';}
...
window.screens['admin-dashboard'] = {
  render() {
    ...
    return `
    <div style="display:flex;flex-direction:column;min-height:100%;background:#080810;color:#fff;">
      ${_adminMiniHeader()}
      <div style="flex:1;overflow-y:auto;padding:16px;">
        ...
        <!-- Alerta usuarios en riesgo (reales) -->
        ${s.atRisk.length > 0 ? `
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px;">
          ...
          <div style="background:#0a0a12;border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:14px;text-align:center;">
            <div style="font-size:22px;font-weight:800;color:#fbbf24;">${s.totalUsers}</div>
...
/*CONTENIDO ADMIN (Retos + Logros + Ejercicios)*/
